...
""" 
This script is responsible for running all trials for a single environment.

It is invoked only by the metaorchestrator and it manages the following:
    pod lifecycle,
    cgroup monitoring and metric collection,
...
class CgroupHandler:
    """
    This class discovers the cgroup v2 path for a given pod by querying containerd for the host PID and reading /proc/[pid]/cgroup,
    then exposes a method to read the relevant cgroup files and extract CPU and memory metrics.
    """
    
    def __init__(self, pod_name: str, namespace: str, env: dict) -> None:
        self.path = None
    ...
    def _find_path(self, pod_name: str, namespace: str, env: dict) -> None:
...
def kubectl(cmd_args: list, env: dict | None=None, capture_output: bool=True, check: bool=False, stdin: str | None=None) -> subprocess.CompletedProcess:
...
def kubectl_output(cmd_args: list, env: dict | None=None) -> str | None:
    try:
...
# self explanatory
def kubectl_pod_exists(pod_name: str, namespace: str, env: dict | None=None) -> bool:
...
class PodOrchestrator:
    """
    This class orcestrates the full trial loop for a single environment, 
    managing pod lifecycle, cgroup monitoring, log streaming and result saving.
    """
    ...
    # if it gets killed mid-write the original results file stays intact.
    ...
    def create_pod_yaml(self, pod_name: str, size: int, duration: float, warmup: bool) -> dict:
    ...
    def prepare_pod(self, pod_name: str, manifest: dict, env: dict) -> tuple[str, CgroupHandler, float, float]:
        # 1. cleanup
    ...
    def monitor_cgroup(self, cgroup: CgroupHandler, interval: float, stop_event: threading.Event, samples: list) -> None:
    ...
    def compute_additional_metrics(self, trial_entry: dict) -> tuple[dict | None, str | None]:
    ...
    def _execute_benchmark(self, pod_name: str, env: dict, cgroup: CgroupHandler, stop_event: threading.Event, samples: list, phase_ts: dict) -> str:
    ...
    def _process_and_save_results(self, trial_idx: int, matrix_size: int, suffix: str, phase_ts: dict, raw_output: str, samples: list) -> None:
    ...
    def _cleanup_pod(self, pod_name: str, env: dict) -> None:
    ...
    def run_trial(self, trial_idx: int, matrix_size: int, warmup: bool) -> None:
